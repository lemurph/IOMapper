...
func _ready():
	...
	dev.set_value_float("physics", 0, 1)
	dev.set_value_vector3("RGB", 0, Vector3(0.0, 0.0, 0.0))


func _process(delta):
	dev.poll()
	sleep = dev.get_value_int("physics", 0)
	get_node("../..").set_sleeping(sleep)
	var rgb_vector = dev.get_value_vector3("RGB", 0)
